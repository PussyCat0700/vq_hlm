...
class GPT2Model(GPT2PreTrainedModel):
    ...
    def __init__(self, config, input_layers=None, vqvae=None, ctx_layers=None, chunk_size=None):
    ...
    def load_pretrained_embedding(self, file_path):
        # 加载预训练权重并将其赋值给embedding层
        checkpoint = self.load_saved_params(file_path)
        
        # 假设文件中存储的embedding权重是'weight'，你可以根据实际情况调整
        pretrained_embedding_weight = checkpoint['embedding.weight']
        
        # 确保维度匹配
        if pretrained_embedding_weight.size(0) == self.codebook_size and pretrained_embedding_weight.size(1) == self.embedding_dim:
            self.embedding.weight.data = pretrained_embedding_weight
        else:
            print(f"Warning: The pretrained embedding weight size does not match the expected size "
                  f"({self.codebook_size}, {self.embedding_dim})")
...
class GPT2LMHeadModel(GPT2PreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config, input_layers=None, ctx_layers=None, vae_model=None, training_type=None):
        ...
        if vae_model != None:
            if vae_model['vae_pretrained_model_path']:
                self.vae_model = get_model(vae_model['vae_config_path'])
                load_checkpoint(self.vae_model, None, vae_model['vae_pretrained_model_path'])
            else:
                self.vae_model = get_model(vae_model['vae_config_path'])
        else: vae_model = None
        self.input_layers = input_layers
        ...
        if self.vae_model != None:
            ...
            self.transformer = GPT2Model(config, self.input_layers, vqvae, self.ctx_layers, chunk_size=self.chunk_size)
        else:
    ...
    def forward(
        ...
        if labels is not None:
            ...
            gpt_loss = loss
            # 'after_input_layer', 'except_codebook', 'ours'
